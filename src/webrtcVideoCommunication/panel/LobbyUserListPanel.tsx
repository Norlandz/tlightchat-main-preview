...
export const LobbyUserListPanel: React.FC<{ signalserverWebsocketClientId_self_sessionReactApp: SignalserverWebsocketClientId }> = ({ signalserverWebsocketClientId_self_sessionReactApp }) => {
  ...
  React.useEffect(() => {
    ...
    initRun.socketioClient_forWebrtcConnection.socket.on(SignalserverWebsocketMsgType.lobbyUserList, socketio_listener);
    return () => {
      initRun.socketioClient_forWebrtcConnection.socket.off(SignalserverWebsocketMsgType.lobbyUserList, socketio_listener);
    };
  }, [dispatch]);                              
  ...
  return (
    <div id={styles.cssId_lobbyUserList}>
      lobbyUserList:
      <ul>
              ...
              <ul>
                {Array.from(mpp_webrtcConnectionAnchorId_peer, ([webrtcConnectionAnchorId_peer, lobbyUserInfo_peer]) => {
                  ...
                  return (
                    ...
                    >
                      <code>{webrtcConnectionAnchorId_peer}</code> <code>{lobbyUserInfo_peer.customName}</code> <code>{lobbyUserInfo_peer.lobbyUserStatus}</code>{' '}
                      <code>{webrtcConnectionAnchor_self?.webrtcConnectionAnchorLocation_peer ? 'connected *<' : ''}</code>{' '}
                      {signalserverWebsocketClientId_peer === signalserverWebsocketClientId_self_sessionReactApp ? null : (
                        <button
                          onClick={async function select_webrtcConnectionAnchorLocation_peer() {
                            dispatch(slice_webrtcConnectionAnchorLocation_peer_currSel.actions.select_webrtcConnectionAnchorLocation_peer(webrtcConnectionAnchorLocation_peer));
                          }}
                        >
